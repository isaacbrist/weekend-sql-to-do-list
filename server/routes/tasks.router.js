...
router.get('/', (req, res) => {
  ...
  let queryText = 'SELECT * FROM "to-do-table" ORDER BY "id";';
  pool
    .query(queryText)
    .then((result) => {
...
router.post('/', (req, res) => {
  ...
  let queryText = `INSERT INTO "to-do-table" ("name", "description")
                   VALUES ($1, $2);`;
  pool
    .query(queryText, [newTask.name, newTask.description])
    .then((result) => {
      res.sendStatus(201);
    })
    .catch((error) => {
